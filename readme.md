...
# Equilibrated files
The `equilibrated-tpr-cpt-files` folder contains equilibrated GROMACS run input (`.tpr`) and checkpoint (`.cpt`) files for zero-field DPPC and DOPC lipid bilayer simulations. For each bilayer system, 40 distinct replicas with the same simulation box dimensions are provided. All replicas were equilibrated for 20 ns in the NVT ensemble.
